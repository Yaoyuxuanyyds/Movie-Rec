...
class MoviesLoader(BasicDataset):
    ...
    def __init__(self, path="../data/ml-latest", test_ratio=0.1, seed=2020):
        ...
        # Create weights based on ratings
        filtered_ratings['weight'] = filtered_ratings['rating'].apply(lambda x: (x - 3) / 2.0)  # Normalize to range [0, 1]
        ...
        self.trainWeight = train_data['weight'].to_numpy()  # Add weights
        ...
        print(f"Number of movies: {len(self.movie_map)}")
        ...
        self.UserItemNet = csr_matrix(
            (self.trainWeight, (self.trainUser, self.trainItem)),
            shape=(len(self.user_map), len(self.movie_map))
        )
        ...
        # Pre-calculate
        self._allPos = self.getUserPosItems(list(range(len(self.user_map))))
    ...
    def getSparseGraph(self):
        ...
        weights = torch.FloatTensor(self.trainWeight)  # Use weights

        # Construct adjacency matrix
        first_sub = torch.stack([user_dim, item_dim + self.n_users])
        second_sub = torch.stack([item_dim + self.n_users, user_dim])
        index = torch.cat([first_sub, second_sub], dim=1)
        data = torch.cat([weights, weights])  # Symmetric weights for undirected graph
